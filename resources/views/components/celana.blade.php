...
@foreach ($barangs as $barang)
    @if(isset($barang->kategori) && strtolower($barang->kategori) == 'celana')
    <div class="border border-3 pt-5 pb-5 rounded-[5%]">
         <div class="w-fit">
    <div class="relative h-48 sm:h-64 overflow-hidden">
        <img src="{{url('img/product')}}/{{ $barang->gambar }}" alt="{{ $barang->nama_barang }}" class="w-full h-full object-cover">
    </div>
    ...
    <div class="p-4 space-y-2 text-center items-center">
        ...
        <a href="{{ url('pesan') }}/{{ $barang->id }}">
            <x-button class="w-full justify-center text-sm m-3 text-center">
                Beli Sekarang
            </x-button>
        </a>
